...
resource "aws_security_group_rule" "this" {
  ...
  lifecycle {
    ...
    precondition {
      ...
      error_message = "All VPC account IDs must match the aws provider account ID for IPv4 Intra VPC Security Group Rules."
    }

    precondition {
      condition     = alltrue([for this in var.intra_vpc_security_group_rule.vpcs : contains([local.region_name], this.region)])
      error_message = "All VPC regions must match the aws provider region for IPv4 Intra VPC Security Group Rules."
    }
  }
}
