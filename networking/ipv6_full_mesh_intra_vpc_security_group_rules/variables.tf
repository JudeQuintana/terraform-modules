...
variable "ipv6_full_mesh_intra_vpc_security_group_rules" {
  ...
  validation {
    condition = length(distinct([
      for this in var.full_mesh_intra_vpc_security_group_rules.one.ipv6_intra_vpc_security_group_rules : this.region
    ])) <= 1
  ...
  validation {
    condition = length(distinct([
      for this in var.full_mesh_intra_vpc_security_group_rules.one.ipv6_intra_vpc_security_group_rules : this.account_id
    ])) <= 1
  ...
  validation {
    condition     = alltrue([for this in var.full_mesh_intra_vpc_security_group_rules.one.ipv6_intra_vpc_security_group_rules : length(this.vpcs) > 1])
  ...
  validation {
    condition = length(distinct([
      for this in var.full_mesh_intra_vpc_security_group_rules.two.ipv6_intra_vpc_security_group_rules : this.region
    ])) <= 1
  ...
  validation {
    condition = length(distinct([
      for this in var.full_mesh_intra_vpc_security_group_rules.two.ipv6_intra_vpc_security_group_rules : this.account_id
    ])) <= 1
  ...
  validation {
    condition = length(distinct([
      for this in var.full_mesh_intra_vpc_security_group_rules.three.ipv6_intra_vpc_security_group_rules : this.region
    ])) <= 1
  ...
  validation {
    condition = length(distinct([
      for this in var.full_mesh_intra_vpc_security_group_rules.three.ipv6_intra_vpc_security_group_rules : this.account_id
    ])) <= 1
    error_message = "The Intra VPC Security Group Rules for Three must have the same account id as each other and the aws.three provider alias for IPv6 Full Mesh Intra VPC Security Group Rules."
  }

  validation {
    condition     = alltrue([for this in var.full_mesh_intra_vpc_security_group_rules.three.ipv6_intra_vpc_security_group_rules : length(this.vpcs) > 1])
  ...
  validation {
    condition = [
      for this in var.full_mesh_intra_vpc_security_group_rules.one.ipv6_intra_vpc_security_group_rules : this.rule
      ] == [
      for this in var.full_mesh_intra_vpc_security_group_rules.two.ipv6_intra_vpc_security_group_rules : this.rule
      ] && [
      for this in var.full_mesh_intra_vpc_security_group_rules.two.ipv6_intra_vpc_security_group_rules : this.rule
      ] == [
      for this in var.full_mesh_intra_vpc_security_group_rules.three.ipv6_intra_vpc_security_group_rules : this.rule
      ] && [
      for this in var.full_mesh_intra_vpc_security_group_rules.three.ipv6_intra_vpc_security_group_rules : this.rule
      ] == [
      for this in var.full_mesh_intra_vpc_security_group_rules.one.ipv6_intra_vpc_security_group_rules : this.rule
