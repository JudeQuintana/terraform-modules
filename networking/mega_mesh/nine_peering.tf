########################################################################################
# Begin Mega Mesh nine TGW to One TGW
########################################################################################

resource "aws_ec2_transit_gateway_peering_attachment" "this_nine_to_this_one" {
  provider = aws.nine
...
resource "aws_ec2_transit_gateway_peering_attachment_accepter" "this_nine_to_this_one" {
  provider = aws.one

  transit_gateway_attachment_id = aws_ec2_transit_gateway_peering_attachment.this_nine_to_this_one.id
  tags = merge(
    local.default_tags,
    {
      Name = format(local.peering_name_format, local.one_tgw.full_name, local.nine_tgw.full_name)
      Side = "One Accepter"
    }
  )
}
...
########################################################################################
# Begin Mega Mesh nine TGW to Two TGW
########################################################################################

resource "aws_ec2_transit_gateway_peering_attachment" "this_nine_to_this_two" {
  provider = aws.nine
...
resource "aws_ec2_transit_gateway_peering_attachment_accepter" "this_nine_to_this_two" {
  provider = aws.two

  transit_gateway_attachment_id = aws_ec2_transit_gateway_peering_attachment.this_nine_to_this_two.id
  tags = merge(
    local.default_tags,
    {
      Name = format(local.peering_name_format, local.two_tgw.full_name, local.nine_tgw.full_name)
      Side = "Two Accepter"
    }
  )
}
...
########################################################################################
# Begin Mega Mesh nine TGW to Three TGW
########################################################################################

resource "aws_ec2_transit_gateway_peering_attachment" "this_nine_to_this_three" {
  provider = aws.nine
...
resource "aws_ec2_transit_gateway_peering_attachment_accepter" "this_nine_to_this_three" {
  provider = aws.three

  transit_gateway_attachment_id = aws_ec2_transit_gateway_peering_attachment.this_nine_to_this_three.id
  tags = merge(
    local.default_tags,
    {
      Name = format(local.peering_name_format, local.three_tgw.full_name, local.nine_tgw.full_name)
      Side = "Three Accepter"
    }
  )
}
...
########################################################################################
# Begin Mega Mesh nine TGW to Four TGW
########################################################################################

resource "aws_ec2_transit_gateway_peering_attachment" "this_nine_to_this_four" {
  provider = aws.nine
...
resource "aws_ec2_transit_gateway_peering_attachment_accepter" "this_nine_to_this_four" {
  provider = aws.four

  transit_gateway_attachment_id = aws_ec2_transit_gateway_peering_attachment.this_nine_to_this_four.id
  tags = merge(
    local.default_tags,
    {
      Name = format(local.peering_name_format, local.four_tgw.full_name, local.nine_tgw.full_name)
      Side = "Four Accepter"
    }
  )
}
...
########################################################################################
# Begin Mega Mesh nine TGW to Five TGW
########################################################################################

resource "aws_ec2_transit_gateway_peering_attachment" "this_nine_to_this_five" {
  provider = aws.nine
...
resource "aws_ec2_transit_gateway_peering_attachment_accepter" "this_nine_to_this_five" {
  provider = aws.five

  transit_gateway_attachment_id = aws_ec2_transit_gateway_peering_attachment.this_nine_to_this_five.id
  tags = merge(
    local.default_tags,
    {
      Name = format(local.peering_name_format, local.five_tgw.full_name, local.nine_tgw.full_name)
      Side = "Five Accepter"
    }
  )
}
...
########################################################################################
# Begin Mega Mesh nine TGW to six TGW
########################################################################################

resource "aws_ec2_transit_gateway_peering_attachment" "this_nine_to_this_six" {
  provider = aws.nine
...
resource "aws_ec2_transit_gateway_peering_attachment_accepter" "this_nine_to_this_six" {
  provider = aws.six

  transit_gateway_attachment_id = aws_ec2_transit_gateway_peering_attachment.this_nine_to_this_six.id
  tags = merge(
    local.default_tags,
    {
      Name = format(local.peering_name_format, local.six_tgw.full_name, local.nine_tgw.full_name)
      Side = "Six Accepter"
    }
  )
}
...
########################################################################################
# Begin Mega Mesh nine TGW to seven TGW
########################################################################################

resource "aws_ec2_transit_gateway_peering_attachment" "this_nine_to_this_seven" {
  provider = aws.nine
...
resource "aws_ec2_transit_gateway_peering_attachment_accepter" "this_nine_to_this_seven" {
  provider = aws.seven

  transit_gateway_attachment_id = aws_ec2_transit_gateway_peering_attachment.this_nine_to_this_seven.id
  tags = merge(
    local.default_tags,
    {
      Name = format(local.peering_name_format, local.seven_tgw.full_name, local.nine_tgw.full_name)
      Side = "Seven Accepter"
    }
  )
}
...
########################################################################################
# Begin Mega Mesh nine TGW to eight TGW
########################################################################################

resource "aws_ec2_transit_gateway_peering_attachment" "this_nine_to_this_eight" {
  provider = aws.nine
...
resource "aws_ec2_transit_gateway_peering_attachment_accepter" "this_nine_to_this_eight" {
  provider = aws.eight

  transit_gateway_attachment_id = aws_ec2_transit_gateway_peering_attachment.this_nine_to_this_eight.id
  tags = merge(
    local.default_tags,
    {
