...
variable "mega_mesh" {
  ...
  validation {
    condition = length(
      distinct([var.mega_mesh.one.centralized_router.name, var.mega_mesh.two.centralized_router.name, var.mega_mesh.three.centralized_router.name, var.mega_mesh.four.centralized_router.name, var.mega_mesh.five.centralized_router.name, var.mega_mesh.six.centralized_router.name])
    ) == length([var.mega_mesh.one.centralized_router.name, var.mega_mesh.two.centralized_router.name, var.mega_mesh.three.centralized_router.name, var.mega_mesh.four.centralized_router.name, var.mega_mesh.five.centralized_router.name, var.mega_mesh.six.centralized_router.name])
    error_message = "All Centralized Routers must have a unique names across regions."
  }

  validation {
    condition = length(
      distinct([var.mega_mesh.one.centralized_router.amazon_side_asn, var.mega_mesh.two.centralized_router.amazon_side_asn, var.mega_mesh.three.centralized_router.amazon_side_asn, var.mega_mesh.four.centralized_router.amazon_side_asn, var.mega_mesh.five.centralized_router.amazon_side_asn, var.mega_mesh.six.centralized_router.amazon_side_asn])
  ...
  validation {
    condition     = length(distinct([var.mega_mesh.one.centralized_router.account_id, var.mega_mesh.two.centralized_router.account_id, var.mega_mesh.three.centralized_router.account_id, var.mega_mesh.four.centralized_router.account_id, var.mega_mesh.five.centralized_router.account_id, var.mega_mesh.six.centralized_router.account_id])) <= 1
