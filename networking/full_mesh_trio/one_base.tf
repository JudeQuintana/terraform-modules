...
locals {
  ...
  one_tgw_vpc_network_cidrs      = toset(concat(local.one_tgw.vpc.network_cidrs, local.two_tgw.vpc.secondary_cidrs))
  one_tgw_vpc_ipv6_network_cidrs = toset(compact(local.one_tgw.vpc.ipv6_network_cidrs))
