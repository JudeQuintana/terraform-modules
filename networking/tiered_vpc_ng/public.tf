locals {
  ...
  public_subnet_cidr_to_tags        = merge([for this in var.tiered_vpc.azs : zipmap(this.public_subnets[*].cidr, this.public_subnets[*].tags)]...)
...
resource "aws_subnet" "this_public" {
  ...
  tags = merge(
    lookup(local.public_subnet_cidr_to_tags, each.key),
    local.default_tags,
    {
      Name = format(
        "%s-%s-%s-%s-%s",
