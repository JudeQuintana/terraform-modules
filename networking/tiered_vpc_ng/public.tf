...
locals {
  ...
  public_any_subnet_cidr_exists     = length(local.public_subnet_cidrs) > 0
...
resource "aws_route_table_association" "this_public" {
  ...
  route_table_id = lookup(aws_route_table.this_public, local.public_any_subnet_cidr_exists).id
}
