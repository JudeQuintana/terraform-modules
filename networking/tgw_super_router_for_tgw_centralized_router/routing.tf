...
locals {
  ...
  local_tgw_route_table_id_to_local_tgw_id = zipmap(local.local_tgws_all_vpc_routes[*].route_table_id, local.local_tgws_all_vpc_routes[*].transit_gateway_id)
...
resource "aws_route" "this_local_vpc_routes_to_peer_tgws" {
  ...
  transit_gateway_id     = lookup(local.local_tgw_route_table_id_to_local_tgw_id, each.value.route_table_id)
}
...
locals {
  # keep track of current rtb-id to tgw-id
  peer_tgw_route_table_id_to_peer_tgw_id = zipmap(local.peer_tgws_all_vpc_routes[*].route_table_id, local.peer_tgws_all_vpc_routes[*].transit_gateway_id)
...
resource "aws_route" "this_peer_vpc_routes_to_local_tgws" {
  ...
  route_table_id         = each.value.route_table_id
  destination_cidr_block = each.value.destination_cidr_block
  transit_gateway_id     = lookup(local.peer_tgw_route_table_id_to_peer_tgw_id, each.value.route_table_id)
}
