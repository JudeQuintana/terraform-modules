...
locals {
  peer_tgws_vpc_network_cidrs_and_route_table_ids_with_tgw_id = [
  ...
  peer_vpc_network_cidr_to_peer_tgw_id = merge([
    for this in local.peer_tgws_vpc_network_cidrs_and_route_table_ids_with_tgw_id : {
      for vpc_network_cidr in this.vpc_network_cidrs :
      vpc_network_cidr => this.transit_gateway_id
  }]...)

  peer_vpc_ipv6_network_cidr_to_peer_tgw_id = merge([
    for this in local.peer_tgws_vpc_network_cidrs_and_route_table_ids_with_tgw_id : {
  ...
  peer_tgws_vpc_network_cidrs      = flatten(local.peer_tgws_vpc_network_cidrs_and_route_table_ids_with_tgw_id[*].vpc_network_cidrs)
  peer_tgws_vpc_ipv6_network_cidrs = flatten(local.peer_tgws_vpc_network_cidrs_and_route_table_ids_with_tgw_id[*].vpc_ipv6_network_cidrs)
  peer_tgws_vpc_route_table_ids    = flatten(local.peer_tgws_vpc_network_cidrs_and_route_table_ids_with_tgw_id[*].vpc_route_table_ids)
  peer_tgws_route_table_ids        = local.peer_tgws[*].route_table_id
  peer_tgws_ids                    = local.peer_tgws[*].id
}

########################################################################################
# IPv4 Begin Peer Centralized Router Side
#########################################################################################
...
resource "aws_ec2_transit_gateway_route" "this_peer" {
  ...
  transit_gateway_attachment_id  = lookup(aws_ec2_transit_gateway_peering_attachment.this_peer_to_peers, each.value).id
...
locals {
  ...
  peer_current_vpc_routes = flatten([
    for this in local.peer_tgws : [
      for route_table_id_and_vpc_network_cidr in setproduct(lookup(local.peer_tgws_vpc_tgw_id_to_route_table_ids, this.id), concat(this.vpc.network_cidrs, this.vpc.secondary_cidrs)) : {
...
########################################################################################
# IPv4 Begin Peer Super Router Side
#########################################################################################
...
########################################################################################
# IPv6 Begin Peer Centralized Router Side
#########################################################################################

# add all peer tgw ipv6 routes to peer tgw super router
resource "aws_ec2_transit_gateway_route" "this_peer_ipv6" {
  provider = aws.peer
...
locals {
  ...
  peer_vpc_ipv6_routes_to_local_tgws = [
    for route_table_id_and_peer_tgw_ipv6_network_cidr in setproduct(local.peer_tgws_vpc_route_table_ids, local.local_tgws_vpc_ipv6_network_cidrs) : {
      ...
      destination_cidr_block = route_table_id_and_peer_tgw_ipv6_network_cidr[1]
  }]
...
locals {
  ...
  peer_current_vpc_ipv6_routes = flatten([
    for this in local.peer_tgws : [
      for route_table_id_and_vpc_ipv6_network_cidr in setproduct(lookup(local.peer_tgws_vpc_tgw_id_to_route_table_ids, this.id), concat(this.vpc.ipv6_network_cidrs, this.vpc.ipv6_secondary_cidrs)) : {
        route_table_id         = route_table_id_and_vpc_ipv6_network_cidr[0]
        destination_cidr_block = route_table_id_and_vpc_ipv6_network_cidr[1]
  }]])
...
locals {
  ...
  peer_tgw_all_new_tgw_ipv6_routes_to_vpcs_in_local_tgws = {
    for this in local.peer_tgw_ipv6_routes_to_local_tgws :
    format(local.route_format, this.route_table_id, this.destination_cidr_block) => this
  }
}

resource "aws_ec2_transit_gateway_route" "this_peer_tgw_ipv6_routes_to_vpcs_in_peer_tgws" {
  provider = aws.peer

  for_each = local.peer_tgw_all_new_tgw_ipv6_routes_to_vpcs_in_local_tgws
...
locals {
  ...
  peer_tgw_all_new_tgw_ipv6_routes_to_peer_tgws = {
    for this in setsubtract(local.peer_tgws_ipv6_routes_to_peer_tgws, local.peer_current_tgw_ipv6_routes) :
    format(local.route_format, this.route_table_id, this.destination_cidr_block) => this
  }
}

resource "aws_ec2_transit_gateway_route" "this_peer_tgw_ipv6_routes_to_peer_tgws" {
  provider = aws.peer

  for_each = local.peer_tgw_all_new_tgw_ipv6_routes_to_peer_tgws
...
########################################################################################
# IPv6 Begin Peer Super Router Side
#########################################################################################

# add all local tgw ipv6 routes to peer tgw super router
resource "aws_ec2_transit_gateway_route" "this_peer_ipv6_to_local_tgws" {
  provider = aws.peer
