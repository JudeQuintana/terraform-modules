locals {
  ...
  centralized_egress_central_vpc_name_to_route_any_cidr = {
    for this in local.vpcs :
    this.name => local.centralized_egress_route_any_cidr
...
resource "aws_ec2_transit_gateway_route" "this_centralized_egress_tgw_central_vpc_route_any" {
  for_each = local.centralized_egress_central_vpc_name_to_route_any_cidr
